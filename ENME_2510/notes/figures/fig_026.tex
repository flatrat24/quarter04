...
\begin{tikzpicture}
  ...
  % Beam
  \filldraw[fill=fg!10!bg, draw=fg, thick] (2,0) rectangle ++(3,0.5);
  ...
  \filldraw[fill=gr, draw=gr] (e) circle (3pt) node[anchor = west] {$A$};
  ...
  % Internal Forces
  \draw[draw=gr!50!fg, ultra thick, stealth-] ($(e)+(-0.25,0)$) -- ++(-0.5,0) node[anchor = east] {$F_{Ix}$};
  \draw[draw=gr!50!fg, ultra thick, -stealth] ($(e)+(-0.15,-1)$) -- ++(0,2) node[anchor = east] {$F_{Iy}$};
  \draw[draw=gr!25!fg, ultra thick, dashed] ($(e)+(-1,-0.9)+(-0.35,-0.25)$) node[anchor = west, xshift = 10pt] {$M_I$} -- (e);
  \draw[-stealth,gr!50!fg, ultra thick] ($(e)+(-1,-0.9)$) arc [
    start angle = 30,
    end angle = 330,
    x radius = 0.5,
    y radius = 0.5
  ...
  % Lengths
  % \draw[draw=fg, thick, |-|] (0.1,-0.2) -- node[anchor = north] {$L_1$} ++(1.9,0);
