...
\begin{tikzpicture}
  ...
  \filldraw[fill=bl, draw=bl] (e) circle (3pt) node[anchor = west] {$A$};

  % Reaction Forces
  \draw[draw=bl!50!fg, ultra thick, stealth-] (-0.25,0.25) -- ++(-0.5,0) node[anchor = east] {$F_{Rx}$};
  \draw[draw=bl!50!fg, ultra thick, -stealth] (-0.15,-0.75) -- ++(0,2) node[anchor = east] {$F_{Ry}$};
  \coordinate (d) at (-0.5,-0.75);
  \draw[draw=bl!25!fg, ultra thick, dashed] ($(d)+(-0.45,-0.35)$) node[anchor = east, xshift = -10pt] {$M_R$} -- (0,0.25);
